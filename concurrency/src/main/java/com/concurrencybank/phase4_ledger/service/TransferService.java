...
/**
 * DB-layer sibling of {@code phase2_deadlock.LockOrderedTransferService}: same
 * lock-ordering principle (always take the lock on the lower account id
 * first), except the lock is a Postgres row lock instead of a
 * {@code ReentrantLock}, and the transaction manager holds it for the whole
 * {@code @Transactional} method instead of a manual try/finally.
 *
 * <p>Two concurrent transfers between the same pair of accounts, in either
 * direction, always request the row locks in the same order — so the second
 * transaction simply blocks on the {@code SELECT ... FOR UPDATE} until the
 * first commits and releases its locks, instead of the two transactions
...
public class TransferService {
    ...
    public TransferResponse transfer(Long fromAccountId, Long toAccountId, long amountMinor) {
        ...
        Account second =
                accounts.findByIdForUpdate(secondId).orElseThrow(() -> new AccountNotFoundException(secondId));
        ...
        // No explicit save(): both accounts are managed entities inside this
        // transaction, so Hibernate's dirty checking flushes the changes on commit.
