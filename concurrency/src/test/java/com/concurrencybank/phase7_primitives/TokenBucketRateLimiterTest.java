...
class TokenBucketRateLimiterTest {
    ...
    /**
     * The regression test for the remainder bug described in the class javadoc. Callers poll far more often than one
     * token-interval, and every one of those polls runs {@code refill()} on a sub-token amount of elapsed time. If
     * {@code refill()} snapped its marker forward to {@code now} on those calls it would discard a tenth of a second
     * each time here and the token would never arrive at all.
     */
    @Test
    ...
    void concurrentCallersCannotTakeMoreThanTheBucketHolds() throws InterruptedException {
        // The clock never moves, so no refill is possible and capacity is a hard ceiling on total grants however many
