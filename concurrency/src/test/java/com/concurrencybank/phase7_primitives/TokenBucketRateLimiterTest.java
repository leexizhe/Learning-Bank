...
/**
 * Every test here drives a <b>fake clock</b> instead of sleeping. A rate limiter
 * tested with {@code Thread.sleep} asserts on the scheduler's mood and fails on a
 * loaded CI box; an injected clock makes refill behaviour exact and instant.
 * Same idea as the postgres module's {@code afterRead} / {@code duringHold}
 * seams — make the race deterministic rather than probable.
 */
class TokenBucketRateLimiterTest {
    ...
    /**
     * The regression test for the remainder bug described in the class javadoc.
     * Callers poll far more often than one token-interval, and every one of those
     * polls runs {@code refill()} on a sub-token amount of elapsed time. If
     * {@code refill()} snapped its marker forward to {@code now} on those calls it
     * would discard a tenth of a second each time here and the token would never
     * arrive at all.
     */
    @Test
