...
class SlidingWindowRateLimiterTest {
  ...
  /**
   * The reason this class exists alongside {@link TokenBucketRateLimiter}, and the answer to "why
   * isn't a fixed window enough?".
   *
   * <p>A fixed-window counter ("5 per second, reset on the second") would let all five of these
   * through at 999ms and five more at 1001ms — ten requests in two milliseconds, every one of them
   * inside the stated policy. The sliding window still counts the first batch, so it cannot.
   */
  @Test
