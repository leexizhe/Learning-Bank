...
class TellerQueueTest {
    ...
    /**
     * Producers and consumers have to run genuinely concurrently (the queue is smaller than the total item count, so
     * producers block on a full queue until consumers drain it) - {@code testutil.ConcurrencyHarness} doesn't fit here,
     * it's built for "N threads doing the same independent task", not two cooperating groups, so this test manages its
     * own threads.
     */
    @Test
