...
/**
 * Pinning affects <em>scalability</em>, not correctness - a pinned virtual thread still runs the critical section under
 * full mutual exclusion, it just hogs a carrier thread while doing it. So this test can't observe pinning itself (see
 * the class javadoc on {@link PinningDemo} for how to); it only confirms both locking strategies are still race-free.
 */
