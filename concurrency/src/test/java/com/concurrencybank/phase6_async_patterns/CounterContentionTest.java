...
class CounterContentionTest {
  ...
  /**
   * Both counters are exact. That is the assertion; the timings are a printout, deliberately. What
   * the relative numbers actually are depends on core count, JIT warmup and machine load, so a
   * threshold here would flake in CI without teaching anything - the same "assert the mechanism,
   * don't time it" rule the postgres module's {@code NPlusOneIT} follows when it counts statements
