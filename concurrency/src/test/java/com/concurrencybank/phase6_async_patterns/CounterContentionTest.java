...
class CounterContentionTest {
     ...
     * numbers actually are depends on core count, JIT warmup and machine load, so a threshold here would flake in CI
     * without teaching anything - the same "assert the mechanism, don't time it" rule the postgres module's {@code
     * NPlusOneIT} follows when it counts statements instead of milliseconds.
     */
    @Test
