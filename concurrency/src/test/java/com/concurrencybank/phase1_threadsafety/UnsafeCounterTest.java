...
class UnsafeCounterTest {
    ...
    /**
     * Demonstrates the bug before showing the fix: {@code balance += amount} is not atomic, so with enough concurrent
     * depositors some increments get lost and the final balance ends up lower than the arithmetic sum. This is the
     * "before" picture — {@link SynchronizedAccountTest} and {@link AtomicAccountTest} run the identical stress shape
     * and prove the fixed versions never lose an update.
     */
    @Test
