...
/**
 * The centerpiece integration test: it never inspects locks or transactions
 * directly, it just proves the observable guarantee. 200 real HTTP transfer
 * requests hit a real Postgres (Testcontainers) concurrently, alternating
 * direction between the same two accounts — exactly the deadlock trap shape
 * from {@code phase2_deadlock}, now exercised through the full Spring MVC +
 * JPA stack. If {@code TransferService}'s lock ordering were wrong, this test
 * would either hang (deadlock) or leave the books unbalanced (lost update).
 */
class TransferControllerIT extends BaseControllerIT {
    ...
    /**
     * Response type is {@code String}, not {@code TransferResponse}: on the
     * error paths the body is a ProblemDetail JSON object, which can't be
     * deserialized into {@code TransferResponse}'s primitive fields. Reading
     * the raw body avoids a message-converter exception masking the assertion
     * this test actually cares about (the status code).
     */
    ...
    void concurrentOppositeDirectionTransfersStayBalanced() throws InterruptedException {
        ...
        AccountResponse bobAfter =
                rest.getForEntity(baseUrl() + "/api/accounts/" + bob.id(), AccountResponse.class)
                        .getBody();
