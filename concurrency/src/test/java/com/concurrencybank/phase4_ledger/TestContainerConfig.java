...
/**
 * Singleton container for the whole test JVM: started once in a static block, shared by every IT.
 * {@code schema.sql} runs on every context start (see {@code spring.sql.init.mode=always}), so it
 * uses {@code CREATE TABLE IF NOT EXISTS} to stay idempotent against a reused container.
 */
