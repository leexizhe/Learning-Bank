...
class StringLockBugDemoTest {
    ...
    void unrelatedOperationsSerializeBecauseTheySharedAnInternedStringLock() throws InterruptedException {
        ...
        Thread ledgerThread = new Thread(() -> {
            try {
        ...
        // Two logically-unrelated 100ms operations should overlap and take ~100ms. Because both synchronize on the same
        // interned literal, they instead serialize and take ~200ms.
    ...
    void lockStripedRegistryLetsUnrelatedKeysRunConcurrently() throws InterruptedException {
        ...
        Thread ledgerThread =
                new Thread(() -> runQuietly(() -> registry.runExclusively("ledger", () -> Thread.sleep(WORK_MILLIS))));
        auditThread.start();
        ledgerThread.start();
        auditThread.join();
        ledgerThread.join();
