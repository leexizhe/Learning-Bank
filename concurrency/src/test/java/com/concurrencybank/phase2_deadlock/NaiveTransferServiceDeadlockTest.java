 ...
 *
 * <p>Most deadlock "tests" assert that the <em>fixed</em> version finishes quickly, which is weak
 * evidence — a slow machine looks like a deadlock and a lucky interleaving looks like a fix. This
 * one asserts the opposite direction: the broken implementation genuinely deadlocks, and the
 * witness is {@code ThreadMXBean.findDeadlockedThreads()} — the JVM's own lock-graph analysis, not
 * a timeout.
 *
 * <p><b>The two threads started here never finish.</b> That is the point, and it dictates the rest
 * of the design: they are <b>daemon</b> threads and are never {@code join()}ed, so the Surefire
 * fork can still exit at the end of the run. A non-daemon deadlocked thread would hang the build
 * permanently — which is exactly the production failure being demonstrated, and a poor property for
 * a test to have.
 */
class NaiveTransferServiceDeadlockTest {
  ...
  /**
   * Daemon, and deliberately never joined — see the class javadoc. Named so that a thread dump
   * taken during the run reads clearly.
   */
