...
class StopFlagVisibilityTest {
    ...
    /**
     * The anomaly, so it gets a printout rather than an assertion — see
     * {@code UnsafePublicationTest} for the full reasoning.
     *
     * <p>Whether the JIT hoists the plain read out of the loop depends on how
     * long the loop ran before C2 compiled it, which is not a property a build
     * should depend on in either direction. Asserting it hangs would fail
     * whenever the JIT declines; asserting it terminates would fail whenever it
     * doesn't. So this reports which happened.
     *
     * <p>The reader is a <b>daemon</b> thread and is never joined without a
     * timeout — if the hoist does occur, that thread spins until the JVM exits,
     * and a non-daemon thread would hang the build forever.
     */
    @Test
    void aPlainFlagMayNeverBeNoticed() throws InterruptedException {
        ...
        System.out.println("StopFlagVisibility: the plain-flag reader "
                + (noticed ? "did notice the write (the JIT did not hoist the read this time)" : "never noticed the write - the read was hoisted out of the loop"));
