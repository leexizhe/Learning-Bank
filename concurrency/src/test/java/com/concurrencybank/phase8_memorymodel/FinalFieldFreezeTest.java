...
class FinalFieldFreezeTest {
    ...
    /**
     * The same race as {@code UnsafePublicationTest}, against the same plain publishing field, differing only in that
     * the holder's field is {@code final}. Here the anomaly is not merely unlikely but forbidden — the freeze at the
     * end of the constructor emits a store-store barrier, so the field write can never be reordered past the reference
     * publication — which is why this one gets a real assertion rather than a printout.
     */
    @Test
