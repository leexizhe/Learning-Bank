...
/**
 * Asks the JVM's own deadlock detector whether a set of threads is stuck in a cycle. This is what turns "my test timed
 * out, so probably a deadlock" into "the runtime itself reports a circular wait between these two specific threads" — a
 * much stronger claim, and the difference between a test that hangs the build and a test that names the bug.
 *
 ...
 * java.util.concurrent.locks.ReentrantLock}. Only the former walks ownable synchronizers too, so only the former can
 * see this repo's deadlock.
 */
public final class DeadlockProbe {
    ...
    /**
     * Currently-deadlocked thread ids, as a set rather than the raw {@code long[]}-or-{@code null} the JMX API returns
     * — an empty set is far easier to assert against than a null.
     */
     ...
     *
     * <p>Polling is genuinely the right tool here, unlike elsewhere in this module where a latch would be: the threads
     * being watched are — by construction — never going to reach a rendezvous again, so there is nothing for them to
     * signal with. The only way to learn they are stuck is to ask a third party, repeatedly. There is also a real gap
     * between releasing them and the cycle forming, because each still has to travel from "barrier tripped" to "parked
     * on the second lock".
     *
    ...
    public static Set<Long> awaitDeadlockAmong(Set<Long> threadIds, Duration timeout) throws InterruptedException {
        ...
        throw new AssertionError("expected threads "
                + threadIds
                + " to deadlock within "
                + timeout
