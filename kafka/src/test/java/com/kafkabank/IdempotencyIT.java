 ...
 *
 * <p>Kafka gives <b>at-least-once</b> delivery. A consumer that commits its offset only after
 * finishing the work (which is what {@code MANUAL_IMMEDIATE} buys us) can still crash in the window
 * between "database committed" and "offset committed" — and then the broker redelivers a message
 * the application has already applied. For a payment, applying it twice means debiting a customer
 * twice.
 *
 * <p>These tests publish directly rather than through the REST API, because the API generates a
 * fresh {@code eventId} per call — and controlling the eventId is the whole point: identical ids
 * are a redelivery, distinct ids are two payments.
 */
