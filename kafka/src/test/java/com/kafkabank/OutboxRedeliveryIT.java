 ...
 *
 * <p><b>The scenario.</b> A payment is applied and its result committed to the
 * outbox, then the process dies before the send is confirmed. The offset was
 * never acknowledged, so Kafka redelivers the event. Under the old design that
 * redelivery was actively harmful: the idempotency check would correctly refuse
 * to debit twice and, in doing so, guarantee the result could never be produced
 * again. Here it is a no-op and the result is recovered anyway, because
 * recovering it was never the consumer's job.
 *
 * <p><b>How the crash is simulated.</b> Not by killing the JVM, but by flipping
 * {@code published} back to false. That is exactly the state a crash between a
 * successful send and the commit recording it would leave behind, and exactly
 * the state the relay exists to recover from.
 *
 * <p><b>Why this counts records rather than waiting for one.</b> The obvious
 * shape — seek to the end of the topic, then trigger the crash, then wait for a
 * record — is a race against a relay that publishes within about 400ms, and it
 * loses intermittently. Counting matches from offset 0 is monotonic instead:
 * the result must appear <b>twice</b>, once from the original publish and once
 * from the recovery, and no amount of timing can make two records look like one.
 * The payment id is a fresh UUID, so records left on the topic by other test
 * classes are invisible to the filter.
 *
 * <p><b>What was deliberately not done.</b> Standing up a second application
 * context with the relay disabled via {@code @SpringBootTest(properties = ...)}
 * would be a trap here: the containers are static and shared, so the second
 * context's {@code @KafkaListener} joins the <em>same</em> {@code payment-service}
 * group against the same broker. Both contexts would hold partitions, and the
 * first — relay still running — could consume the test's event and publish it,
 * quietly defeating the assertion.
 *
 * <p><b>What this proves, precisely.</b> That the result survives the window and
 * reaches the topic. It does <em>not</em> distinguish which of the two relay
 * routes recovered it — the after-commit listener or the scheduled sweep — and
 * pinning that down would need exactly the disabled-relay context described
 * above. Worth stating rather than implying.
 */
