...
/**
 * The end-to-end happy path across all three roles: HTTP → {@code payment-events} → debit → {@code
 * payment-results} → reconciliation CONFIRMED. Nothing here calls a service method directly; every
 * step is driven by a real record moving through a real broker.
 */
class PaymentFlowIT extends BaseKafkaIT {
  ...
  void paymentFlowsFromHttpThroughKafkaToDebitAndConfirmedReconciliation() {
    ...
    // 202, not 201: at this instant nothing has been debited. All that has
    // happened is a durable append to the topic.
    ...
    // ...and the reconciliation role, in its own consumer group, independently
    // assembles both halves and reaches a terminal state.
    await()
        .atMost(Duration.ofSeconds(30))
        .untilAsserted(
            () -> {
  ...
  void everyEventForOneAccountGetsTheSamePartition() {
    ...
    // hash(key) % partitionCount is deterministic, so the same accountId always
    // resolves to the same partition. That is the entire mechanism behind the
    // per-account ordering guarantee - there is nothing else enforcing it.
