 ...
 *
 * <p>The rule that matters in banking: <b>never silently drop it.</b> A dead-lettered record is a payment some customer
 * believes they made. It goes to a dedicated topic, keeping the original topic/offset and the failure reason as
 * headers, so a human can triage it and a replay job can re-drive it once the underlying bug is fixed.
 *
 * <p>An unknown account extends {@code PermanentFailureException}, which {@code @RetryableTopic} excludes, so it skips
 * the retry topics entirely and dead-letters immediately — retrying a permanently-unknown account would just burn the
