...
/**
 * Proves the per-account ordering guarantee with an outcome that is only
 * possible if the two events were applied in the order they were produced.
 *
 * <p>The trick is picking amounts where order changes the answer. Balances are
 * additive, so "debit 10 then debit 20" ends at the same number either way —
 * that would prove nothing. Instead the account holds exactly enough for the
 * first payment and not the second, so:
 * <ul>
 ...
 * </ul>
 * The final balance alone distinguishes the two. This works because both events
 * share an {@code accountId}, so they share a partition, so one consumer thread
 * handles them in offset order — remove the shared key and this test starts
