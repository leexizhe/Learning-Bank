...
/**
 * The distinction that matters most for how you design a consumer: a <b>rejected</b> payment is not
 * a <b>failed</b> one.
 *
 * <p>Insufficient funds is a correct, final business answer. It must not be retried (retrying
 * declines it again, forever) and it must not be dead-lettered (nothing is broken and no human
 * needs to look at it). It gets a REJECTED result event, the offset is committed, and
 * reconciliation records ROLLBACK. Compare with {@link DeadLetterIT}, where the message genuinely
 * can't be processed.
 */
