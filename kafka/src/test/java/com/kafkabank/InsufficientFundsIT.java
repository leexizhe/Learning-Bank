 ...
 * forever) and it must not be dead-lettered (nothing is broken and no human needs to look at it). It gets a REJECTED
 * result event, the offset is committed, and reconciliation records ROLLBACK. Compare with {@link DeadLetterIT}, where
 * the message genuinely can't be processed.
 */
class InsufficientFundsIT extends BaseKafkaIT {
    ...
    void anUnaffordablePaymentIsRejectedNotRetriedAndReconcilesAsRollback() {
        ...
        // And critically, nothing was dead-lettered - this was an answer, not an error. Drained once, after the outcome
        // above is already settled, rather than inside a retry loop: by this point the decision has demonstrably been
        // made, so a single read is enough and there is nothing to wait for.
