...
/**
 * One row per payment, assembled from two different topics. Keyed by {@code paymentId} — which is
 * exactly why it can be assembled at all: the initiated event and the result event carry the same
 * paymentId, so they can be matched even though they arrive separately, at different times, on
 * different topics, possibly in either order.
 */
@Getter
@Entity
...
public class ReconciliationRecord {
  ...
  /**
   * Only reaches a terminal state once BOTH halves have arrived. Until then it stays PENDING —
   * which is the honest answer, and the thing a reconciliation dashboard actually alerts on: a
   * payment stuck PENDING past some threshold means one of the two sides never showed up.
   */
  private void recomputeState() {
    if (initiatedSeen && resultStatus != null) {
      this.state =
          resultStatus == PaymentStatus.ACCEPTED
