...
/**
 * The read side of a payment. Lives here rather than next to {@code POST /api/payments} because the reconciliation role
...
public class ReconciliationController {
    ...
    /**
     * A 404 here doesn't mean "no such payment" — it can equally mean "that payment is real but neither of its events
     * has been consumed yet". In an eventually consistent read model, absence is a timing statement, not a truth claim.
     */
    @GetMapping("/api/payments/{paymentId}")
    public ReconciliationView getOne(@PathVariable String paymentId) {
        return records.findById(paymentId)
