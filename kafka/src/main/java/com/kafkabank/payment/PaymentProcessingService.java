...
/**
 * All the work for one payment, in one transaction: the idempotency marker, the
 * debit, and the outgoing result event. Three writes, one commit, no window
 * between them.
 *
 * <p><b>This used to return the result for the consumer to publish, and that was
 * a bug.</b> The consumer took the returned value, handed it to an asynchronous
 * {@code KafkaTemplate.send}, discarded the future and committed the offset on
 * the next line. If the broker rejected the send, the offset was already
 * committed and the result was gone with no path to recovery.
 *
 * <p>The obvious fix — block on the send before acknowledging — narrows that
 * window without closing it, and understanding why is worth more than the fix
 * itself. Blocking means a failed send throws, so {@code @RetryableTopic}
 * redelivers the event, so this method runs again, sees its own
 * {@code processed_events} row and correctly refuses to debit twice. Correct —
 * and the result is now <em>permanently</em> unpublishable, because the only
 * thing that could have produced it was the run that already happened. The retry
 * actively makes it worse.
 *
 * <p>Writing the result into the outbox resolves that, and note <em>how</em>:
 * not by returning the previously computed result on a duplicate, but by never
 * needing to. The early return below is safe precisely because the first
 * attempt's outbox row is still there — published already, or waiting for the
 * relay. Redelivery becomes genuinely a no-op instead of a lossy one.
 */
@Slf4j
@Service
