...
/**
 * All the work for one payment, in one transaction: the idempotency marker, the debit, and the
 * outgoing result event. Three writes, one commit, no window between them.
 *
 * <p><b>This used to return the result for the consumer to publish, and that was a bug.</b> The
 * consumer took the returned value, handed it to an asynchronous {@code KafkaTemplate.send},
 * discarded the future and committed the offset on the next line. If the broker rejected the send,
 * the offset was already committed and the result was gone with no path to recovery.
 *
 * <p>The obvious fix — block on the send before acknowledging — narrows that window without closing
 * it, and understanding why is worth more than the fix itself. Blocking means a failed send throws,
 * so {@code @RetryableTopic} redelivers the event, so this method runs again, sees its own {@code
 * processed_events} row and correctly refuses to debit twice. Correct — and the result is now
 * <em>permanently</em> unpublishable, because the only thing that could have produced it was the
 * run that already happened. The retry actively makes it worse.
 *
 * <p>Writing the result into the outbox resolves that, and note <em>how</em>: not by returning the
 * previously computed result on a duplicate, but by never needing to. The early return below is
 * safe precisely because the first attempt's outbox row is still there — published already, or
 * waiting for the relay. Redelivery becomes genuinely a no-op instead of a lossy one.
 */
@Slf4j
@Service
public class PaymentProcessingService {
  ...
  /**
   * @throws UnknownAccountException when the message is unprocessable — triggers retry and
   *     eventually the DLT
   */
  @Transactional
  public void process(PaymentInitiated event) {
    ...
    //
    // Returning early is lossless now: whatever this event produced the first
    // time is sitting in payment_outbox, and publishing it is the relay's job
    // rather than this method's. See the class javadoc.
    ...
    // The third write in this transaction, committing atomically with the debit
    // and the processed_events row. That atomicity is the entire point.
    PaymentOutbox row =
        outbox.save(
            new PaymentOutbox(
                event.eventId(),
                Topics.PAYMENT_RESULTS,
                String.valueOf(event.accountId()),
                toJson(result)));

    // Fired now, delivered only after this transaction commits. Purely a
    // latency shortcut - the scheduled sweep is what makes it correct.
    events.publishEvent(new PaymentResultRecorded(row.getId()));
  }

  /**
   * The accepted and rejected results differ only in status and reason; everything else is copied
   * through.
   */
  ...
  private String toJson(PaymentResult result) {
    try {
      ...
      // If the result can't be serialized the outbox row can't be written,
      // and this transaction must not commit - the alternative is a debit
      // with no corresponding event, which is the very thing being prevented.
      throw new IllegalStateException(
          "Could not serialize result for payment " + result.paymentId(), e);
    }
  }
}
