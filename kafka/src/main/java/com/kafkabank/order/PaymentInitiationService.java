...
/**
 * The producer. This is the "Order Service" of the three-service story: it
 * validates nothing about balances, takes no locks, and touches no ledger — it
 * just records that a customer asked for a payment, durably, and gets out of
 * the way.
 *
 * <p>That's the architectural point worth saying out loud: the synchronous
 * alternative is Order → Payment → Notification → Fraud as a chain of blocking
 * calls, where total latency is the sum of all four and any one of them being
 * down fails the customer's request. Here the API call finishes as soon as
 * Kafka has the event, and every downstream consumer works in parallel.
 */
@Slf4j
@Service
public class PaymentInitiationService {
    ...
    /**
     * Bounds how long a request thread will wait for the broker to acknowledge.
     * Without an explicit timeout, {@code Future.get()} waits up to the producer's
     * {@code delivery.timeout.ms} — two minutes by default. An unreachable broker
     * would then park every incoming request thread for two minutes each, exhaust
     * the servlet pool, and take down endpoints that have nothing to do with
