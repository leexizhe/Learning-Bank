...
/**
 * The database's own deadlock, completing the story {@code WriteSkewIT} starts
 * with SQLSTATE {@code 40001}.
 *
 * <p>The concurrency module deadlocks two Java threads on two {@code ReentrantLock}s
 * and proves it with {@code ThreadMXBean}. This is the same circular wait one
 * layer down, on two database rows — and the interesting difference is what
 * happens next. <b>A JVM deadlock hangs forever; Postgres breaks its own.</b>
 * Every backend that waits longer than {@code deadlock_timeout} (1s by default)
 * runs a wait-graph check, and if it finds a cycle it picks a victim and aborts
 * it with SQLSTATE {@code 40P01}. The survivor commits.
 *
 * <p>That changes what "handle deadlocks" means. In Java you prevent them by
 * lock ordering, because there is no recovery. In Postgres you still prevent
 * them the same way — {@code SELECT ... FOR UPDATE} in ascending id order is the
 * database version of {@code LockOrderedTransferService} — but you must also
 * <em>retry</em>, because the engine will occasionally shoot one of your
 * transactions on purpose. Same retry loop that {@code 40001} needs, which is
 * why both codes belong in the same catch.
 *
 * <p>Which side loses is Postgres's choice, so the test asserts that
 * <b>exactly one</b> transaction failed with {@code 40P01} rather than assuming
 * which. Assuming would make this flaky for no reason.
 */
