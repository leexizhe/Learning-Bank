...
/**
 * The snapshot must be <b>invisible</b>: every assertion here compares it against {@code LedgerService.balanceOf}, the
 * full {@code SUM(postings)} it is meant to accelerate. A cache that returns a different answer from the thing it
 * caches is not an optimization, it is a bug, and in a ledger it is a bug that loses money.
 */
...
class SnapshotIT extends TestContainerConfig {
     ...
     * account, reading the balance through a snapshot touches a fraction of the buffers that summing the journal does.
     */
    @Test
