...
/**
 * A transaction-history endpoint that returns every posting an account has ever made doesn't scale
 * - this asserts the endpoint actually respects {@code page}/{@code size} rather than just
 * accepting the parameters and ignoring them, which is an easy way for pagination to look wired up
 * while quietly still loading everything.
 */
...
class PaginationIT extends TestContainerConfig {
  ...
  void historyEndpointRespectsPageSizeAndReportsCorrectTotals() {
    ...
    postings.saveAll(
        IntStream.range(0, TOTAL_POSTINGS)
            .mapToObj(i -> new Posting(account, null, 1, "history-" + i))
            .toList());

    PostingHistoryController.PagedPostings body =
        rest.getForObject(
