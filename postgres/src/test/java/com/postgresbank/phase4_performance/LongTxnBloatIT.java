 ...
 * could still need to see it. A single long-running transaction anywhere in the system - even one that never touches
 * the table being vacuumed - holds that horizon back for everyone. This opens one connection with a transaction that
 * never commits, updates a row from a second connection, runs {@code VACUUM} while the first transaction is still open
 * (dead tuples survive), then commits the first transaction and vacuums again (they're finally reclaimed).
 */
