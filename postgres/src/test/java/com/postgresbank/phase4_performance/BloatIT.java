...
/**
 * MVCC's other half: every {@code UPDATE} leaves the old tuple version behind
 * as a "dead tuple" (see {@code phase2_ledger.TupleVersionIT}) until
 * something reclaims it. Repeated updates to one row visibly grow
 * {@code pg_stat_user_tables.n_dead_tup}; {@code VACUUM} is what brings it
 * back down. Skip {@code VACUUM} (or block it - see {@link LongTxnBloatIT})
 * and a hot row's table just keeps growing on disk even though its logical
 * row count never changes - that's table bloat.
 */
    ...
    void repeatedUpdatesGrowDeadTuplesAndVacuumReclaimsThem() throws Exception {
        ...
        try (Connection c = dataSource.getConnection()) {
            ...
            Awaitility.await()
                    ...
                    .untilAsserted(() -> assertThat(readStatCounter(c, "accounts", "n_dead_tup"))
