...
/**
 * Same data, same result, two very different query counts. This doesn't
 * trust "JOIN FETCH should be faster" as a rule of thumb - it reads
 * Hibernate's own {@code Statistics} and counts the actual JDBC statements
 * each path executes: 1 (accounts) + N (one lazy postings load per account)
 * for the naive path, exactly 1 for the fetch-joined path.
 */
