 ...
 *
 * <p>Index tests are meaningless on ten rows: Postgres will seq-scan a small table regardless of
 * what indexes exist, and it is right to. These seed ~100k postings so the choice between an index
 * scan and a sequential scan is a real one that the planner makes on cost.
 *
 * <p><b>Two accounts, deliberately lopsided.</b> One holds a couple of hundred rows and one holds
 * essentially all of them. That gives both plan shapes on the same table with no DDL at all: the
 * selective account gets an Index Scan, the bulk account correctly gets a Seq Scan, because reading
 * 99% of a table through an index is slower than just reading the table. The alternative — dropping
 * the index to force a Seq Scan — mutates state every other IT depends on, and teaches less. <b>A
 * sequential scan is sometimes the right answer</b> is the lesson worth having.
 *
 * <p><b>These two accounts break the suite's usual "unique data per test" rule on purpose.</b>
 * Every other test opens {@code "owner-" + UUID.randomUUID()} because the container is reused and
 * never reset. These are found-or-created by fixed name, seeded once per container lifetime, and
 * never mutated by anything — they are read-only fixtures, and re-seeding 100k rows for each test
 * class would dominate the run time for no benefit.
 *
 * <p>Seeded with one {@code INSERT ... SELECT FROM generate_series}, which takes well under a
 * second. Doing it through {@code postings.saveAll()} of 100k entities would take minutes and prove
...
public final class IndexFixture {
  ...
  private static long countPostings(Connection c, long accountId) throws Exception {
    try (PreparedStatement ps =
        c.prepareStatement("select count(*) from postings where account_id = ?")) {
  ...
  /**
   * Distinct, descending {@code created_at} per row, so the ordering tests have something real to
   * order by and keyset pagination has a total order to seek within. amount_minor is 1 rather than
   * 0 because of the {@code postings_amount_nonzero} CHECK.
   */
