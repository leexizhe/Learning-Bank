...
/**
 * A <b>partial index</b> indexes only the rows matching its own {@code WHERE} clause. {@code idx_outbox_unpublished}
 * covers {@code (id) WHERE NOT published}, which is the entire query the relay ever runs.
 *
 * <p><b>Why this shape fits an outbox so exactly.</b> The table grows forever — it is a log — but the interesting set
 * is only ever the handful of rows not yet relayed. A full index on {@code published} would grow with history and be
 * almost entirely made of rows nobody will ever ask about; the partial index stays proportional to the
 * <em>backlog</em>. Same story for the {@code payment_jobs} queue, and for any "pending / unprocessed / not yet sent"
 ...
 * implies the index's. {@code WHERE NOT published} matches; {@code WHERE published = false} also matches; a query with
 * no predicate on {@code published} at all does not, and silently gets a sequential scan.
 */
...
class PartialIndexIT extends TestContainerConfig {
    ...
    /**
     * Seeds bulk history as <b>published</b> only. Unpublished rows would be swept up by the scheduled {@code
     * OutboxRelay} within two seconds anyway, and — more importantly — leaving a backlog behind would change what other
     * ITs in this module see. The plan choice is what is under test, not the row count.
     */
    @BeforeEach
    ...
    /**
     * The other half, and the part that catches people out: the same table, a query that does not mention {@code
     * published}, and the partial index is simply unavailable — it does not contain the rows being asked for.
     */
    @Test
