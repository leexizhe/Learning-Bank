...
/**
 * Composite index column order: <b>equality columns first, then the range or sort column</b>. The query is {@code WHERE
 * account_id = ? ORDER BY created_at DESC, id DESC}, and the claim is that {@code (account_id, created_at, id)} serves
 * it while {@code (created_at, account_id)} does not.
 *
 ...
 * predicate includes its leading column". PG18's B-tree <b>skip scan</b> softens that: the planner can now hop through
 * the distinct values of a leading column it has no predicate for, so the "wrong" index may well be <em>usable</em>
 * here. What skip scan cannot do is hand back rows already sorted by a trailing column. So "the right index removes the
 * Sort, the wrong one doesn't" is true on 16 and on 18, and it is the sharper claim anyway: <b>an index earns its keep
 * by satisfying the ORDER BY, not merely by being touched.</b>
 *
 * <p>A {@code Sort} node on a large result is also where the real cost hides — it has to materialise every matching row
 * before returning the first one, which is exactly what a {@code LIMIT} was supposed to avoid.
 *
 * <p>The deliberately-wrong index is created inside a transaction that is always rolled back, so {@code schema.sql}
 * keeps containing only indexes worth shipping.
 */
    ...
    void theWrongColumnOrderForcesPostgresToSort() throws Exception {
        try (Connection c = dataSource.getConnection()) {
            ...
            try (Statement st = c.createStatement()) {
                ...
                // created_at order, which is a prefix of what the query wants, so Postgres sorts only within each group
                // of equal timestamps rather than sorting everything. Cheaper than a full Sort - and still a sort,
                // which is the point.
                assertThat(plan.hasSortNode())
                        .as(
