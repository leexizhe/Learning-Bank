...
/**
 * {@code LIMIT/OFFSET} versus keyset pagination, compared on <b>buffers read</b> rather than on wall-clock time — the
 * same principle as the rest of this module: measure the work the query asked the storage layer to do, not how busy the
 * machine happened to be.
 *
 * <p><b>Why OFFSET degrades.</b> There is no way to skip rows without producing them. {@code OFFSET 50000} reads fifty
 * thousand rows through the index, throws every one away, and returns the next twenty. The cost is O(offset + size), so
 * page 1 is instant, page 2500 is slow, and the endpoint gets steadily worse in a way that never shows up in testing
 * with 10 rows of data.
 *
 * <p>Keyset instead says "everything strictly after this row", which the B-tree can seek to directly: O(page size) at
 * any depth. The catch is real and worth stating — no page numbers, no total count, no jumping to an arbitrary page —
...
class KeysetPaginationIT extends TestContainerConfig {
     ...
     * ResultSet.getTimestamp().toInstant()}) and then feeding that {@code Instant} back through a Hibernate-bound
     * parameter compares two different interpretations of a {@code TIMESTAMP WITHOUT TIME ZONE} — the driver resolves
     * it against the JVM default zone, Hibernate against its own — and the seek silently lands hours away from where
     * the offset page is. Which is itself the argument for {@code TIMESTAMPTZ} over {@code TIMESTAMP} for anything a
