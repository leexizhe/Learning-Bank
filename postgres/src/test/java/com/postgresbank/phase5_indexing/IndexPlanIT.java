...
/**
 * Asserts on the query plan itself, not on elapsed time — the same discipline as {@code NPlusOneIT}, which counts
 * Hibernate statements rather than timing them. A timing assertion tells you the machine was busy; a plan assertion
 * tells you what the database decided to do and why.
 *
 * <p>The headline is <b>not</b> "indexes are faster". It is that the planner chooses, on cost, per query — and that
...
class IndexPlanIT extends TestContainerConfig {
     ...
     * your answer to "why isn't it using my index?" is always "add a hint", this is the case that should change your
     * mind.
     */
    @Test
    void aBulkAccountLookupCorrectlyPrefersASequentialScan() throws Exception {
        QueryPlan plan = explain(dataSource, "select * from postings where account_id = ?", fixture.bulkAccountId());
    ...
    /**
     * The counterfactual, run inside a transaction that is always rolled back. DDL is transactional in Postgres —
     * unlike MySQL, where it commits implicitly and there is no undo — so dropping the index to prove it matters cannot
     * leak into the shared container even if the assertion fails.
     *
     * <p>Deliberately not {@code SET enable_indexscan = off}: that proves the planner obeys a knob, not that the index
     * is doing anything.
     */
    @Test
