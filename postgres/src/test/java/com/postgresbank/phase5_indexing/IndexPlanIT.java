...
/**
 * Asserts on the query plan itself, not on elapsed time — the same discipline as
 * {@code NPlusOneIT}, which counts Hibernate statements rather than timing them.
 * A timing assertion tells you the machine was busy; a plan assertion tells you
 * what the database decided to do and why.
 *
 * <p>The headline is <b>not</b> "indexes are faster". It is that the planner
 * chooses, on cost, per query — and that <b>a sequential scan is sometimes
 * correct</b>. Both accounts here are on the same table with the same index
...
class IndexPlanIT extends TestContainerConfig {
    ...
    void aSelectiveAccountLookupUsesTheIndex() throws Exception {
        QueryPlan plan = explain(
                dataSource, "select * from postings where account_id = ?", fixture.selectiveAccountId());

        assertThat(plan.usesIndex())
                .as("a couple of hundred rows out of 100k - fetching them by index beats reading the table. Plan was: %s", plan.nodeTypes())
                .isTrue();
        assertThat(plan.actualRows()).isEqualTo(IndexFixture.SELECTIVE_ROWS);
    }

    /**
     * The half everyone forgets. Reading 99% of a table <em>through</em> an index
     * is slower than reading the table, because every index hit is a random jump
     * back into the heap — so the planner correctly declines the index. If your
     * answer to "why isn't it using my index?" is always "add a hint", this is
     * the case that should change your mind.
     */
    @Test
     ...
     * DDL is transactional in Postgres — unlike MySQL, where it commits
     * implicitly and there is no undo — so dropping the index to prove it matters
     * cannot leak into the shared container even if the assertion fails.
     *
     * <p>Deliberately not {@code SET enable_indexscan = off}: that proves the
     * planner obeys a knob, not that the index is doing anything.
     */
    @Test
