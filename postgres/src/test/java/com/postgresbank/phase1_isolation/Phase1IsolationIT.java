...
/**
 * The write-skew anomaly, made concrete: two accounts share one combined overdraft limit. A withdrawal from either
 * account is only allowed if the <em>combined</em> balance can cover it. Two concurrent withdrawals - one per account -
 * can each individually pass that check before either has committed, because neither transaction wrote a row the
 * other's read depended on directly; the conflict is between one transaction's read and the other's write, which READ
 * COMMITTED never detects.
 *
 * <p>Both scenarios force the two transactions to finish their read (and thus their check) before either proceeds to
 * write, using a {@link CountDownLatch} passed in as the {@code afterRead} hook - without that, the anomaly would only
 * reproduce "most of the time," which is a bad property for a test (and a worse one for a bank).
 */
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
class Phase1IsolationIT extends TestContainerConfig {
