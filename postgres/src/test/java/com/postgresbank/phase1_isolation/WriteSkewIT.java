...
/**
 * The write-skew anomaly, made concrete: two accounts share one combined
 * overdraft limit. A withdrawal from either account is only allowed if the
 * <em>combined</em> balance can cover it. Two concurrent withdrawals - one
 * per account - can each individually pass that check before either has
 * committed, because neither transaction wrote a row the other's read
 * depended on directly; the conflict is between one transaction's read and
 * the other's write, which READ COMMITTED never detects.
 *
 * <p>Both scenarios force the two transactions to finish their read (and thus
 * their check) before either proceeds to write, using a {@link CountDownLatch}
 * passed in as the {@code afterRead} hook - without that, the anomaly would
 * only reproduce "most of the time," which is a bad property for a test (and
 * a worse one for a bank).
 */
...
class WriteSkewIT extends TestContainerConfig {
    ...
    @Autowired
    private JointOverdraftService overdraft;
    ...
    void serializableRejectsOneOfTheTwo() throws Exception {
        ...
        assertThat(results).as("exactly one of the two withdrawals should succeed").containsExactlyInAnyOrder(true, false);
        long combined = ledger.balanceOf(a.getId()) + ledger.balanceOf(b.getId());
        assertThat(combined).as("SERIALIZABLE must never let the combined balance go negative").isEqualTo(100 - 80);
    }
    ...
    /** First two calls rendezvous; every call after that is a no-op (later SERIALIZABLE retries don't need it). */
