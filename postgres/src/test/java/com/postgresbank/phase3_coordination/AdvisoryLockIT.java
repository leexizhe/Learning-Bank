...
/**
 * Two concurrent refund attempts for the same order: the first to call {@code pg_try_advisory_xact_lock} wins and holds
 * it for its whole transaction; the second observes the lock unavailable and returns {@code false} immediately - it
 * never blocks or queues, which is the entire reason to reach for {@code try_advisory_lock} over a row lock ({@code
 * SELECT ... FOR UPDATE} would instead make the second caller wait).
 */
