 ...
 * idempotency-key uniqueness check (see that class's javadoc). That ordering is what makes this test meaningful: the
 * losing attempt in a duplicate-key race gets as far as inserting its outbox row before its transfer insert fails - if
 * the outbox write weren't in the same transaction as everything else, that row would survive the rollback as an orphan
 * event describing a transfer that never happened.
 */
...
class OutboxIT extends TestContainerConfig {
    ...
    /**
     * The transactional half, not {@link OutboxRelay} itself. Note that calling this from a test is an <b>external</b>
     * call and therefore goes through the {@code @Transactional} proxy — which is exactly why this test kept passing
     * while the scheduled path, which used to self-invoke, silently did nothing. See {@link
     * OutboxRelayTransactionalOps} for the full story.
     */
    @Autowired
