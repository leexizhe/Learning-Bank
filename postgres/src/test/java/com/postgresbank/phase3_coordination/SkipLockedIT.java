...
/**
 * Seeds a batch of jobs and lets several "worker" threads race to claim them
 * via {@code SELECT ... FOR UPDATE SKIP LOCKED}. Proves the two properties
 * that make this pattern scale to multiple service instances: every job is
 * claimed by exactly one worker (no two workers ever process the same row),
 * and no worker ever blocks waiting on a row another worker already holds -
 * the whole batch drains quickly instead of serializing behind lock waits.
 */
