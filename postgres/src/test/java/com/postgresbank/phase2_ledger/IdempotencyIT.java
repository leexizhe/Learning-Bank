...
/**
 * The UNIQUE constraint on {@code transfers.idempotency_key} is the actual guarantee here, not application logic - two
 * requests racing to insert the same key can only ever have one winner at the database level, no matter how the JVM
 * schedules the two threads. This fires the exact same transfer twice, concurrently, and proves it posts exactly once.
 */
