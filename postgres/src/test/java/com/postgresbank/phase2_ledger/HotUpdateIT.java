...
/**
 * {@code note} on {@code postings} is not indexed (see schema.sql /
 * {@code Posting}). Updating only that column lets Postgres do a HOT
 * (Heap-Only Tuple) update: the new tuple version can be written to the same
 * page without touching any index, because no indexed column changed and
 * there's room on the page. {@code pg_stat_user_tables.n_tup_hot_upd} counts
 * exactly these updates - this test proves the count moves, rather than just
 * asserting the update "worked" (which a plain non-HOT update would also do).
 */
...
class HotUpdateIT extends TestContainerConfig {
    ...
    void updatingANonIndexedColumnRegistersAsHotUpdates() throws Exception {
        ...
        Awaitility.await()
                ...
                .untilAsserted(() -> assertThat(readStatCounter(dataSource, "postings", "n_tup_hot_upd"))
