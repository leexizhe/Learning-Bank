...
/**
 * {@code note} on {@code postings} is not indexed (see schema.sql / {@code Posting}). Updating only that column lets
 * Postgres do a HOT (Heap-Only Tuple) update: the new tuple version can be written to the same page without touching
 * any index, because no indexed column changed and there's room on the page. {@code pg_stat_user_tables.n_tup_hot_upd}
 * counts exactly these updates - this test proves the count moves, rather than just asserting the update "worked"
 * (which a plain non-HOT update would also do).
 */
...
class HotUpdateIT extends TestContainerConfig {
    ...
    void updatingANonIndexedColumnRegistersAsHotUpdates() throws Exception {
        ...
        // pg_stat_user_tables is refreshed from shared memory on commit but a fresh connection's first read can lag it
        // by a beat - poll rather than asserting on a single snapshot.
