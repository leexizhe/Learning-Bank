 ...
 * {@code ctid} is a row's physical address (page number, offset within the page). Postgres never changes a committed
 * tuple's bytes on disk - MVCC needs the old version to stay exactly as it was for any transaction whose snapshot still
 * needs to see it. Update the row and its {@code ctid} moves to a new location, because it's genuinely a new tuple; the
 * old one is left in place, marked expired ({@code xmax} set), waiting for {@code VACUUM} to reclaim its space.
 */
...
class TupleVersionIT extends TestContainerConfig {
    ...
    private void updateAmount(Connection c, long accountId, long newAmountMinor) throws Exception {
        try (PreparedStatement ps = c.prepareStatement("update postings set amount_minor = ? where account_id = ?")) {
