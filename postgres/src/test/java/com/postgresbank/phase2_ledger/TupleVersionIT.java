...
/**
 * Physical proof that an {@code UPDATE} is a delete-and-insert of a new tuple version, never an
 * in-place mutation: {@code ctid} is a row's physical address (page number, offset within the
 * page). Postgres never changes a committed tuple's bytes on disk - MVCC needs the old version to
 * stay exactly as it was for any transaction whose snapshot still needs to see it. Update the row
 * and its {@code ctid} moves to a new location, because it's genuinely a new tuple; the old one is
 * left in place, marked expired ({@code xmax} set), waiting for {@code VACUUM} to reclaim its
 * space.
 */
...
class TupleVersionIT extends TestContainerConfig {
  ...
  void updatingAnIndexedColumnMovesTheTuple() throws Exception {
    ...
    try (Connection c = dataSource.getConnection()) {
      ...
      assertThat(ctidAfter)
          .as(
              "the physical location of the row should change after UPDATE - it's a new tuple version")
  ...
  private String ctidOf(Connection c, long accountId) throws Exception {
    try (PreparedStatement ps =
        c.prepareStatement("select ctid from postings where account_id = ?")) {
      ps.setLong(1, accountId);
