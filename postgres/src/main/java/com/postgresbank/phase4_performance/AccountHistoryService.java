...
/**
 * Two ways to load "these accounts, with their posting history" - same result, very different SQL.
 * {@link #loadNPlusOne} is the mistake: one query for the accounts, then Hibernate silently issues
 * one more query <em>per account</em> the moment {@code getPostings()} is touched, because the
 * association is LAZY (see {@link Account}). {@link #loadFetchJoined} is the fix - a single query
 * with {@code JOIN FETCH} pulls accounts and their postings together. NPlusOneIT proves the query
 * counts directly via Hibernate's {@code Statistics}, rather than just trusting that JOIN FETCH
 * "should" be faster.
 */
@Service
