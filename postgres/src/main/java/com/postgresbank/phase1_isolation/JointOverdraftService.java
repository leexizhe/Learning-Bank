...
/**
 * Public entry point. {@link #withdrawReadCommitted} is left free to reproduce write skew (that's the point - it's the
 * anomaly demo). {@link #withdrawSerializable} is the fix: Postgres detects the conflict itself and fails one of the
 * two transactions with SQLSTATE {@code 40001} ("could not serialize access due to read/write dependencies"). The
 * contract of SERIALIZABLE was never "no one gets it wrong," it's "if it would be wrong, the database tells you and you
 * retry" - so the retry loop here isn't a workaround, it's the other half of the guarantee.
 *
 * <p>Detection reads the SQLState directly off the exception's cause chain rather than trusting a specific
 * Spring/Hibernate exception subtype - which translated type a given driver/ORM version produces for 40001 has shifted
...
public class JointOverdraftService {
    ...
    public void withdrawSerializable(long debitAccountId, long partnerAccountId, long amountMinor, Runnable afterRead) {
        for (int attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
            try {
                ...
                // Retry with fresh reads - this is not the same transaction retrying, it's a brand-new one, exactly as
