...
public class OverdraftController {
    ...
    public void withdraw(@RequestBody WithdrawRequest request) {
        if (request.isolation() == IsolationChoice.SERIALIZABLE) {
            ...
            overdraft.withdrawReadCommitted(request.debitAccountId(), request.partnerAccountId(), request.amountMinor());
        }
    }
    ...
    public record WithdrawRequest(
            long debitAccountId, long partnerAccountId, @Positive long amountMinor, IsolationChoice isolation) {}
}
