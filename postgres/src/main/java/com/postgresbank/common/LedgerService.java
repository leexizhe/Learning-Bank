...
/**
 * The one place a balance is computed. It is never a stored column - always {@code SUM(postings.amount_minor)} for the
 * account, read fresh every time. That's the whole point of an append-only ledger: the "current balance" is a derived
